...
test("full Harness needs only UNITY_EDITOR and defaults its project path to the repository", async () => {
  const result = runHarness(["--full"], { UNITY_EDITOR: process.execPath });
  assert.equal(result.status, 1);
  assert.doesNotMatch(result.stdout, /RAZOR_UNITY_PROJECT/);
  assert.match(result.stderr, /Unity EditMode tests failed with exit code/);

  const source = await readFile(
    path.join(root, "scripts", "harness", "verify.mjs"),
    "utf8"
  );
  assert.match(source, /const projectPath = process\.env\.RAZOR_UNITY_PROJECT \|\| ROOT;/);
