# 设计审查报告 — 旧 RazorFramework 的重构输入

> **当前状态（2026-07-28）：** 本文保留的是从 Unsaid Goodbye 提取的旧框架审计，不是 CardGame 当前玩法架构的验收报告。feat-005 已为 CardGame 建立 Unity `6000.3.10f1` 项目宿主、固定项目身份和 Bootstrap 场景，并可运行 EditMode 配置测试；因此“缺少 Unity 宿主、无法实际验证项目”的问题已解决。
>
> **仍存风险：** 根目录 `Boot/`、`DI/`、`Events/`、`Input/`、`Lifecycle/`、`MVVM/` 仍是未迁入的旧源码，只作为后续重构输入。本阶段刻意不让它们进入 Unity 编译范围，不能把本文中“提取版本已修复”的历史描述当作 CardGame 已验证的运行时结论。DI 的 Unity 空值判断债务仍由 feat-002 跟踪；程序集边界和框架迁入将在后续独立功能中处理。
