...
本文只记录 CardGame 当前已验证的架构结论与尚未解决的风险。基线中的旧 RazorFramework 风险清单已迁存到 [旧框架历史审计快照](docs/legacy-framework-audit.md)，它是 feat-003 的重构输入，不可被当作现有运行时的验收结论。
...
DI V2 在 `Build()` 时冻结注册并验证依赖图；支持 Singleton、具层级关系的 Scoped、Transient、逆序释放、集合解析和非侵入式诊断。Harness 会递归拒绝根 `DI/` 下的任何 C# 源码，拒绝 DI 核心中的 `UnityEngine` 记号或外来命名空间，并在完整模式实际编译并执行 EditMode 测试。生命周期和作用域错误会保留间接 captive、兄弟 scope、集合 scope 和运行时 mismatch 的类型路径。
...
| Unity 注入 | 反射式字段/属性赋值 | Unity 初始化完成后仅在捕获的主线程调用；通过 `[Inject]` 与 `[InjectOptional]` 表达成员依赖 |
